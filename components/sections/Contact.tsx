...
const CONTACT_INFO = [
  {
    icon: Mail,
    title: 'Email',
    value: 'petertoss2004@gmail.com',
    color: 'text-emerald-400',
    bg: 'bg-emerald-500/10',
    href: 'mailto:petertoss2004@gmail.com',
    target: '_self',
  },
  {
    icon: Linkedin,
    title: 'LinkedIn',
    value: 'linkedin.com/in/peter-toss',
    color: 'text-cyan-400',
    bg: 'bg-cyan-500/10',
...
export default function Contact() {
  const [particles, setParticles] = useState<{ id: number; top: number; left: number }[]>([]);
  ...
  const FloatingElement = (props: any) => (
    <motion.div
      {...props}
      className={`${props.className} absolute`}
      transition={{ repeat: Infinity, ease: 'easeInOut', ...props.transition }}
  ...
  return (
    <section
      id="contact"
      className="min-h-screen flex flex-col items-center justify-center bg-gray-950 px-4 md:px-6 py-12 md:py-16 lg:py-20 relative overflow-hidden"
    >
      {/* Animated Background */}
      <div className="absolute inset-0 z-0 pointer-events-none">
        {/* Grid */}
        ...
        {/* Gradient Orbs & Floating Tech Elements */}
        <FloatingElement
          className="top-1/4 left-1/4 w-8 h-8 border border-cyan-400/30 rounded-sm"
          animate={{ rotate: 360 }}
          transition={{ duration: 25, repeat: Infinity, ease: 'linear' }}
        />
        <FloatingElement
          className="bottom-1/3 right-1/4 w-6 h-6 bg-emerald-400/20 rounded-full"
          animate={{ scale: [1, 1.3, 1] }}
          transition={{ duration: 5 }}
        />
        <FloatingElement
          className="top-1/3 right-1/3 w-4 h-4 bg-violet-400/20"
          animate={{ y: [0, -15, 0] }}
          transition={{ duration: 7 }}
        />
        <FloatingElement
          className="bottom-1/4 left-1/3 w-5 h-5 border border-blue-400/30 rounded-full"
          animate={{ x: [0, 25, 0] }}
        ...
        {/* Gradient Orbs */}
        <FloatingElement
          className="top-1/4 left-1/4 w-32 h-32 md:w-48 md:h-48 bg-gradient-to-r from-cyan-500/10 to-blue-500/10 rounded-full blur-2xl"
          animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.5, 0.3] }}
          transition={{ duration: 8 }}
        />
        <FloatingElement
          className="bottom-1/4 right-1/4 w-32 h-32 md:w-56 md:h-56 bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 rounded-full blur-2xl"
          animate={{ scale: [1, 1.15, 1], opacity: [0.2, 0.4, 0.2] }}
          transition={{ duration: 10, delay: 1 }}
        />
        <FloatingElement
          className="top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 md:w-72 md:h-72 bg-gradient-to-r from-violet-500/10 to-blue-500/10 rounded-full blur-2xl"
          animate={{ scale: [1, 1.2, 1], opacity: [0.15, 0.3, 0.15] }}
      ...
      <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 z-10">
        ...
        <motion.div
          ...
          className="text-center mb-16"
        >
          <h2 className="font-bold text-white" style={{ fontSize: 'clamp(2rem, 5.5vw, 2.5rem)' }}>
            Get In Touch
          </h2>
          <div className="w-20 h-1 bg-gradient-to-r from-emerald-500 to-cyan-500 mx-auto mt-4"></div>
          <p className="text-gray-400 mt-6 max-w-2xl mx-auto text-lg leading-relaxed">
          ...
          {CONTACT_INFO.map((item, i) => {
            ...
            return (
              ...
              >
                <div className="flex flex-col sm:flex-row items-center gap-5 p-5 rounded-2xl bg-gray-700/20 hover:bg-gray-700/30 border border-transparent hover:border-emerald-500/30 transition-all duration-300">
                  <div
                    className={`${item.bg} w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110`}
                  >
                    <Icon className={`${item.color}`} size={28} />
                  </div>
                  <div className="flex-1 min-w-0 text-center sm:text-left">
